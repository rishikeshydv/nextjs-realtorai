...
export default function StickyScrolling({
  ...
  const containerProps = useRef<{ height: number; top: number; bottom: number }>({
    height: 0,
    top: 0,
    bottom: 0,
  })
  ...
  useEffect(() => {
    if (containerRef.current) {
      setSections(Array.from(containerRef.current.querySelectorAll('section')).map((el) => el as HTMLElement))
    }
  }, [])  
  ...
  const initContainer = () => {
    if (containerRef.current) {      
      containerRef.current.style.setProperty('--stick-items', `${sections.length + 1}00vh`)
  ...
  const handleSections = () => {
    if (containerRef.current && sections) {
      ...
      containerProps.current.bottom = containerProps.current.top + containerProps.current.height
      ...
      if (containerProps.current.bottom <= viewportTop.current) {
        ...
        scrollValue.current = remapValue(viewportTop.current, containerProps.current.top, containerProps.current.bottom, 0, sections.length + 1)
      }
      activeIndex.current = Math.floor(scrollValue.current) >= sections.length ? sections.length - 1 : Math.floor(scrollValue.current)
